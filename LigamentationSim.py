...
# Binary tree
class Node:
    def __init__(self, size, d, smin, yfac=[0], parent=None):
        ...
        if (d>0):
            if (size>smin):
                ...
                # Build tree
                self.left = Node(self.size*yr, d-1, smin, self.yfac+[yl*yl*size], self)
                self.right = Node(self.size*yl, d-1, smin, self.yfac+[-yr*yr*size], self)

                # Add nodes to levels
                Levels[depth-d].append([self.right, self.left])

            else: # Minimum size is reached, do not split
                self.right = Node(self.size, d-1, smin, self.yfac+[0], self)
                Levels[depth-d].append([self.right])
...
# Animation function
def animate(window, canvas, radius=50, vel=50, d=2, smin=30, scale=1, slow=1, colour = '#ffa500'):
    # Update global variables
    global depth, Levels
    depth = d
    Levels = [[] for i in range(0, depth+1)]

    root = Node(radius, depth, smin) # Create Tree with initial radius and depth
    ...
    for frame in range(1, depth*60):
        canvas.delete('all')                 # Clear canvas
        for shape in Levels[frame//60]:      # For each shape/shape-pair in current level
            if (len(shape)==2):              # Split happened
            ...
            else: # No split, display single particle
                # Getting info for the particle
                [r1, yfac1] = shape[0].getInfo()
...
in1 = tk.Entry(fr1, width=15)
in1.insert(tk.END, '65')
lb1.pack(side=tk.LEFT)
in1.pack(side=tk.LEFT, padx=(0, 11))

lb2 = tk.Label(fr1, text='xVelocity:')
in2 = tk.Entry(fr1, width=15)
in2.insert(tk.END, '150')
lb2.pack(side=tk.LEFT)
in2.pack(side=tk.LEFT, padx=(0, 11))

lb3 = tk.Label(fr1, text='No. of Split:')
in3 = tk.Entry(fr1, width=15)
in3.insert(tk.END, '3')
lb3.pack(side=tk.LEFT)
in3.pack(side=tk.LEFT, padx=(0, 11))

lb4 = tk.Label(fr1, text='Min. Radius:')
in4 = tk.Entry(fr1, width=15)
in4.insert(tk.END, '30')
lb4.pack(side=tk.LEFT)
in4.pack(side=tk.LEFT, padx=(0, 11))

lb5 = tk.Label(fr1, text='Frame Scale:')
in5 = tk.Entry(fr1, width=15)
in5.insert(tk.END, '0.5')
lb5.pack(side=tk.LEFT)
in5.pack(side=tk.LEFT, padx=(0, 11))

lb6 = tk.Label(fr1, text='Time Scale:')
in6 = tk.Entry(fr1, width=15)
in6.insert(tk.END, '1')
lb6.pack(side=tk.LEFT)
in6.pack(side=tk.LEFT, padx=(0, 0))

# Run onClick function
def onClick():
    animate(window=window, canvas=canvas, radius=float(in1.get()), vel=float(in2.get()), d=int(in3.get()), smin=float(in4.get()), scale=float(in5.get()), slow=float(in6.get()))

# Info onClick function
def infoCl():
    messagebox.showinfo('Details', 'Radius(float) - Initial Radius of the particle\nxVelocity(float) - x velocity of particle\nNo. of Split(int) - Max. No of times particle will split\nMin. Radius - Minimum radius for splitting\nFrame Scale(float) - Scale of coordinate axes\nTime Scale(float) - Scale of animation speed\n\nOptimal values are entered by default.')
